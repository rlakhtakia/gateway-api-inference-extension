...
func GenerateStreamedRequestSet(logger logr.Logger, prompt, model string, filterMetadata interface{}) []*extProcPb.ProcessingRequest {
...
func GenerateRequestMetadata(filterMetadata interface{}) map[string]*structpb.Struct {
	metadata := make(map[string]*structpb.Struct)
	if filterMetadata != nil {
		structVal, _ := structpb.NewStruct(map[string]interface{}{
			"x-gateway-destination-endpoint-subset": filterMetadata,
		})
